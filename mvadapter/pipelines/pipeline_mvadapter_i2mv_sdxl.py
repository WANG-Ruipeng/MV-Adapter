    ...
    def __call__(
        ...
        **kwargs,
        ...
        Args:
            ...
            reference_generator (`torch.Generator` or `List[torch.Generator]`, *optional*):
                Independent generator used only when sampling the reference-image VAE posterior. Defaults to
                `generator` for backward compatibility. Supplying it keeps default and externally generated
                initial-latent experiments on an identical reference-conditioning random stream.
            scheduler_generator (`torch.Generator` or `List[torch.Generator]`, *optional*):
                Independent generator used by stochastic scheduler steps. Defaults to `generator` for backward
                compatibility. Supplying it prevents different latent constructors from shifting per-step noise.
        ...
        reference_latents = self.prepare_image_latents(
            ...
            prompt_embeds.dtype,
            device,
            reference_generator if reference_generator is not None else generator,
            add_noise=False,
        )
